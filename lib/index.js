...
class ToPDF {
        ...
        if (options.cacheImage) {
            page.on('response', async (response) => {
                ...
                if (response.request().resourceType() === 'image') {
                    ...
                    fs_1.default.writeFileSync(file, new Uint8Array(buffer));
    ...
    loadPDF(file) {
        return pdf_lib_1.PDFDocument.load(new Uint8Array(fs_1.default.readFileSync(file)));
    }
    writeLargeFile(file, data) {
        const fd = fs_1.default.openSync(file, 'w');
        const CHUNK = 512 * 1024 * 1024;
        try {
            for (let offset = 0; offset < data.length; offset += CHUNK) {
                const len = Math.min(CHUNK, data.length - offset);
                fs_1.default.writeSync(fd, data, offset, len);
            }
        }
        finally {
