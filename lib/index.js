...
class ToPDF {
    ...
    getChunkPath(i) {
        const { options } = this;
        return path_1.default.join(options.outputPath, `${options.name}${i + 1}.pdf`);
    }
    async downloadPdf() {
        ...
        for (let i = 0; i < groups.length; i++) {
            ...
            if (options.cacheChunk && fs_1.default.existsSync(chunkPath)) {
                continue;
            }
            await page.goto(`http://127.0.0.1:${this.port}/?page=${i + 1}`, {
                waitUntil: 'networkidle0'
            });
            const height = await page.evaluate(() => document.images[0].height || document.body.scrollHeight);
    ...
    async mergePDF() {
        ...
        for (let i = 0; i < groups.length; i++) {
            let document = await pdf_lib_1.PDFDocument.load(fs_1.default.readFileSync(this.getChunkPath(i)));
